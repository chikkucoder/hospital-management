  ...
  return (
    <div className="space-y-10">
       <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-[#06402B] tracking-tighter">Your Health Hub</h1>
          <p className="text-gray-500 font-medium italic">Welcome back, {name}. Your stats look <span className="text-emerald-600 font-bold">Excellent</span>.</p>
        </div>
        <button className="h-14 px-8 bg-[#06402B] text-white rounded-2xl font-bold flex items-center gap-2 shadow-xl shadow-emerald-600/20 hover:scale-105 transition-transform">
           <Plus className="w-5 h-5" /> Book Appointment
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard title="Next Visit" value="Apr 24" icon={Calendar} color="text-emerald-600" />
        <StatCard title="Pulse" value="72 BPM" icon={Activity} color="text-red-500" />
        <StatCard title="Medicines" value="2 Active" icon={Pill} color="text-blue-600" />
        <StatCard title="Lab Status" value="Pending" icon={FlaskConical} color="text-orange-600" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-10">
             <h3 className="text-xl font-bold text-[#06402B] tracking-tight uppercase tracking-widest text-xs opacity-50">Recent History</h3>
             <button className="text-xs font-bold text-emerald-600 hover:underline">Download All Records</button>
          </div>
          <div className="space-y-12 relative before:absolute before:left-3.5 before:top-4 before:bottom-0 before:w-px before:bg-gray-100">
            {[
        ...
        <div className="space-y-6">
              ...
              <div className="space-y-4">
                 {[
                   ...
                   <div key={i} className="p-4 bg-gray-50 rounded-2xl flex items-center justify-between">
                      ...
                      <span className="text-[9px] font-black bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full uppercase tracking-tighter">
                         {m.days}
                      </span>
                   </div>
                 ))}
           ...
           <div className="bg-emerald-600 p-10 rounded-[3rem] text-white shadow-xl shadow-emerald-900/10 flex flex-col justify-between min-h-[300px] group">
              <div>
                 <Plus className="w-12 h-12 mb-8 text-emerald-200 opacity-50 group-hover:rotate-90 transition-transform duration-500" />
              ...
              <button className="w-full py-4 bg-white text-emerald-600 font-black rounded-2xl shadow-xl shadow-emerald-900/20 hover:scale-105 transition-transform">
                 Connect Now
              </button>
           </div>
        </div>
